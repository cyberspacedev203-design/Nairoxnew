...
// Send a welcome email directly via Brevo.
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!userName) userName = (toEmail || '').split('@')[0];
    ...
    if (!sendRes.ok) {
      ...
      console.error('Brevo welcome send failed', sendRes.status, text);
      return res.status(502).json({ error: 'Failed to send welcome email', details: text });
    }
